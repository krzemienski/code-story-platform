...
import { ChevronLeft, ChevronRight, Play, Pause, Loader2, Mic, Film, Sparkles, GraduationCap, Cpu } from "lucide-react"
...
export function TailsCarousel({ tails }: TailsCarouselProps) {
  ...
  const goTo = (index: number) => {
    if (isAnimating || tails.length === 0) return
    setIsAnimating(true)
  ...
  const goNext = () => {
    ...
    goTo((currentIndex + 1) % tails.length)
  }

  const goPrev = () => {
    if (tails.length === 0) return
    goTo((currentIndex - 1 + tails.length) % tails.length)
  }

  // Auto-advance
  useEffect(() => {
    if (tails.length <= 1) return
    const timer = setInterval(goNext, 6000)
    return () => clearInterval(timer)
  }, [currentIndex, tails.length])

  if (tails.length === 0) {
    return (
      <div className="text-center py-12 text-muted-foreground">
        <p>No featured tails yet. Be the first to generate one!</p>
      </div>
    )
  }
      ...
      <div ref={carouselRef} className="relative overflow-hidden rounded-2xl">
        ...
        >
          {tails.map((tail, index) => (
            <TailCard key={tail.id} tail={tail} isActive={index === currentIndex} />
          ))}
        </div>
      </div>
...
function TailCard({ tail, isActive }: { tail: Tail; isActive: boolean }) {
  const { play, currentItem, isPlaying, isBuffering, toggle } = useAudioPlayerContext()
  const styleConfig = getStyleConfig(tail.narrative_style)

  const isThisPlaying = currentItem?.id === tail.id && isPlaying
  const isThisBuffering = currentItem?.id === tail.id && isBuffering
  const hasAudio = tail.audio_url || (tail.audio_chunks && tail.audio_chunks.length > 0)
  ...
  const handlePlay = (e: React.MouseEvent) => {
    ...
    if (!hasAudio) {
      window.location.href = `/story/${tail.id}`
      return
    }

    if (currentItem?.id === tail.id) {
      toggle()
    } else {
      play({
        id: tail.id,
        title: tail.title,
        subtitle: tail.narrative_style,
        repoName: tail.code_repositories
          ? `${tail.code_repositories.repo_owner}/${tail.code_repositories.repo_name}`
          : undefined,
        audioUrl: tail.audio_url || undefined,
        audioChunks: tail.audio_chunks || undefined,
        duration: tail.actual_duration_seconds || undefined,
      })
    }
  }
  ...
  const generateWaveformHeights = () => {
    const hash = tail.title.split("").reduce((a, b) => ((a << 5) - a + b.charCodeAt(0)) | 0, 0)
        ...
        >
          ...
          <div className="p-6 sm:p-8">
            ...
            <p className="text-sm text-muted-foreground mb-2">
              {tail.code_repositories && (
                <>
                  By <span className="text-foreground">@{tail.code_repositories.repo_owner}</span> ·{" "}
                </>
              )}
              {tail.code_repositories?.primary_language || "Code"} · {formatDuration(tail.actual_duration_seconds)}{" "}
              runtime
            </p>

            {tail.code_repositories?.description && (
              <p className="text-sm text-muted-foreground line-clamp-2 mb-6">{tail.code_repositories.description}</p>
            )}
